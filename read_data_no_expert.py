...
from models.fumend_from_gate_value import MultiDomainFENDModel
...
path1 = Path('/home/casiie/lsl/MDFEND/data')
path2 = Path('/home/casiie/lsl/MDFEND/state_dict')
# path3 = Path('/home/yanzhou/NewModel/ProcessData')
split = [[], [], []]  # 划分训练集、测试集， 11826
split_list = pd.read_csv('/home/casiie/lsl/MDFEND/data/split.csv')  # （229580，2）
label = pd.read_csv('/home/casiie/lsl/MDFEND/data/label.csv')  # （11826，2）用户ID，标签
domain_path = Path('/home/casiie/lsl/MDFEND/data')
...
class InterDataset(Dataset):
    def __init__(self, name='train'):
        ...
        tmp = np.load('/home/casiie/lsl/MDFEND/data/gate_value.npy')
        ...
        self.tweet_feature = torch.load(path1 / 'tweet_embedding.pt') # torch.Size([11826, 200, 768])
        self.des_feature = torch.load(path1 / 'des_embedding.pt') # self.des_feature
        self.edge_index = torch.load(path1 / 'edge_index_1.pt')  # check if tensor, torch.Size([2, 455958])
        print('load data...')
        self.text = torch.load('/home/casiie/lsl/MDFEND/data/text_no_fine.pt')  # 11826 torch.Size([11826, 201, 768])
    ...
    def __getitem__(self, index):
        return self.text[index], self.domain[index], self.user_neighbor_index[index], \
               self.user_label[index], self.num_feature, self.cat_feature,  self.tweet_feature, self.des_feature,self.edge_index
...
def my_collate(batch):  # len(batch[0]) : 8
    ...
    tweet_feature = batch[0][6] # torch.Size([11826, 200, 768])
    des_feature = batch[0][7] # torch.Size([11826, 768])
    edge_index = batch[0][8]  # torch.Size([2, 455958])
    return [text, domain, user_neighbor_index, user_label, num_feature, cat_feature,tweet_feature,des_feature, edge_index]
        ...
        for epoch in range(self.num_epochs):
            ...
            with tqdm(self.train_loader) as progress_bar:
                for batch in progress_bar:
                    ...
                    tweet_feature = batch[6].to(self.device)
                    des_feature = batch[7].to(self.device)
                    edge_index = batch[8].to(self.device)
                    pred = self.model(text, domain, user_neighbor_index, num_feature, cat_feature,tweet_feature,des_feature,
            ...
            if max_acc < val_acc:
                a = open('/home/casiie/lsl/MDFEND/result/result_model.txt', 'a')
    ...
    def val(self):
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                des_feature = batch[7].to(self.device)
                edge_index = batch[8].to(self.device)
                pred = self.model(text, domain, user_neighbor_index, num_feature, cat_feature,tweet_feature,des_feature, edge_index)
                loss = self.loss_func(pred, user_label.float())
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch in self.test_loader:
                ...
                pred = self.model(text, domain, user_neighbor_index, num_feature, cat_feature,tweet_feature,des_feature, edge_index).cpu().numpy()
...
if __name__ == '__main__':
    ...
    trainer = InterTrainer(train_loader, val_loader, test_loader, model=MultiDomainFENDModel,
                           ...
                           state_dict_path='/home/casiie/lsl/MDFEND/state_dict/state_dict_model.tar',
                           device='cuda:0')
    trainer.train()
