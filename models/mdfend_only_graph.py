...
class InteractModel(nn.Module):
    def __init__(self, num_property_dim=6, cat_property_dim=11, tweet_dim=768,
        ...
        self.final_linear = nn.Linear(44, output_dim)  # 128 -- 128
    ...
    def forward(self, text, user_neighbor_index, num_feature, cat_feature, edge_index):
        ...
        # 注意力权重、文本交互、图交互
        final_input = self.final_linear(user_feature)  # torch.Size([64, 128])
...
class MultiDomainFEND_Only_Graph_Model(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, dropout):
        super(MultiDomainFEND_Only_Graph_Model, self).__init__()
...
if __name__ == '__main__':
    model = MultiDomainFEND_Only_Graph_Model(768, [384], 0.2)
