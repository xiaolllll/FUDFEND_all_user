...
class InteractModel(nn.Module):
    ...
    def forward(self, text, user_neighbor_index, num_feature, cat_feature, edge_index):
        ...
        # 注意力权重、文本交互、图交互
        final_input = self.final_linear(title)  # torch.Size([64, 128])
...
class MultiDomainFEND_Only_Text_Model(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, dropout):
        super(MultiDomainFEND_Only_Text_Model, self).__init__()
...
if __name__ == '__main__':
    model = MultiDomainFEND_Only_Text_Model(768, [384], 0.2)
