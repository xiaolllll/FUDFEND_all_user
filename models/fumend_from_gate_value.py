...
class InteractModel(nn.Module):
    def __init__(self, num_property_dim=6, cat_property_dim=11, tweet_dim=768,
        ...
        self.final_linear = nn.Linear(output_dim, output_dim)  # 128 -- 128
    ...
    def forward(self, text, user_neighbor_index, num_feature, cat_feature, edge_index):
        ...
        # 注意力权重、文本交互、图交互
        final_input = torch.cat((attention_vec, title, user_feature), dim=-1)  # torch.Size([64, 128])
...
class MultiDomainFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, dropout):
        ...
        self.gate = nn.Sequential(nn.Linear(128 + self.domain_num, mlp_dims[-1]),
                                  nn.ReLU(),
        ...
        self.rnn = nn.LSTM(768, 128, batch_first=True)
        # self.rnn_2 = nn.LSTM(768, 128, batch_first=True)
        self.rnn_fc_1 = nn.Linear(128, 128)

    def forward(self, text, category, user_neighbor_index, num_feature, cat_feature, edge_index):

        domain_embedding, _ = self.rnn(text)
        # domain_embedding, _ = self.rnn_2(text)
        batch_size, seq_len, mid_dim = domain_embedding.shape
        domain_embedding = domain_embedding[:, -1, :]
        domain_embedding = domain_embedding.view(batch_size, mid_dim)
        domain_embedding = self.rnn_fc_1(domain_embedding)

        one_user_tweet = domain_embedding

        gate_input = torch.cat([category, one_user_tweet], dim=-1)
